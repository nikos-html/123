...
// Get doc_id and token from URL
var params = new URLSearchParams(window.location.search);
var docId = params.get('doc_id');
var viewToken = params.get('token') || sessionStorage.getItem('view_token');
...
// SECURITY: If no data in sessionStorage but we have doc_id, fetch from server WITH TOKEN
if (Object.keys(data).length === 0 && docId) {
  // Build URL with token for security
  ...
  fetch(fetchUrl)
    .then(response => {
      if (response.status === 404) {
        // SECURITY: Document doesn't exist - show error from backend
      ...
      if (response.status === 403) {
        ...
        document.body.innerHTML = '<div style="display: flex; justify-content: center; align-items: center; height: 100vh; background: #09020e; color: #ff3333; font-size: 1.5rem; font-family: Inter, sans-serif; text-align: center;">🔒 Brak dostępu do tego dokumentu</div>';
