...
TURNSTILE_SECRET_KEY = os.environ.get('TURNSTILE_SECRET_KEY', '0x4AAAAAACIETFZW2JPN4TewZskRq48ujK4')
...
async def verify_turnstile(token):
    """SECURITY: Verify Cloudflare Turnstile token"""
...
def admin_login():
    ...
    # SECURITY: Turnstile REQUIRED for admin login
    turnstile_token = data.get('turnstile_token')
    if TURNSTILE_SECRET_KEY:
        if not turnstile_token or not verify_turnstile_sync(turnstile_token):
...
def create_document_with_id():
    ...
    try:
        ...
        cur.execute(
            ...
            RETURNING id
        ''',
            (user_id, name, surname, pesel, access_code, json.dumps(data)))
...
def generate_codes():
    try:
        ...
        for i in range(count):
            ...
            # SECURITY: Insert with expiration
            cur.execute('INSERT INTO one_time_codes (code, expires_at) VALUES (%s, %s)', (code, expires_at))
            codes.append(code)
        ...
        logger.info(f"Admin generated {len(codes)} access codes")
        return jsonify({'codes': codes, 'expires_at': expires_at.isoformat()}), 201
    except Exception as e:
...
def get_codes():
    try:
        conn = get_db()
        cur = conn.cursor(row_factory=dict_row)
        cur.execute('SELECT id, code, used, used_at, created_at, expires_at FROM one_time_codes ORDER BY created_at DESC')
        codes = cur.fetchall()
        cur.close()
        conn.close()
